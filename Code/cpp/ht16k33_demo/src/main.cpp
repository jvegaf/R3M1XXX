...
// Ajusta 'segmentMap' si el cableado físico del módulo no coincide con ese
// orden. segmentMap[i] = índice de bit físico (0..7) que controla el segmento
// lógico i. Mapeo confirmado: bits físicos 0..7 corresponden directamente a
// a,b,c,d,e,f,g,dp Ajusta si cambias el hardware.
...
static const uint8_t digitTableLogical[] = {0x3F, 0x06, 0x5B, 0x4F, 0x66, 0x6D,
...
uint8_t encodeSegments(uint8_t logicalPattern) {
  ...
  if (!segmentsActiveHigh) {
    // Invertimos sólo a..g (sin DP). Usa 0xFF si también quieres invertir el
    // punto decimal.
...
uint8_t encodeDigit(uint8_t d) {
  if (d < 16)
    return encodeSegments(digitTableLogical[d]);
  return 0;
}
...
void showNumber(uint8_t displayNum, int number) {
  ...
  displayBuffer[comOffset * 2] =
      (hundreds > 0 || number >= 100) ? encodeDigit(hundreds) : 0x00;
  if (tens > 0 || number >= 10) {
...
void runDemoStep() {
  ...
  if (now - lastDemoUpdate < demoInterval)
    return;
  lastDemoUpdate = now;

  // Secuencia simple: 0-99 iguales, 100-199 invertidos, 200-299 separados,
  // 300-309 todos segmentos, 310-317 segmentos individuales
  if (demoCounter < 100) {
...
const bool includeDecimalPointInTest =
    false; // pon a true si quieres probar el punto decimal
...
void showCurrentSegment() {
  ...
  Serial.print(" (");
  Serial.print(segmentNames[logicalSegment]);
  Serial.print(") -> físico esperado? ");
  ...
  Serial.print(segmentsActiveHigh ? "sí" : "no");
  Serial.print(" | Avance=");
  Serial.println(autoAdvance ? "auto" : "manual");
}
...
void showPhysicalBit(uint8_t physBit) {
  ...
  updateDisplay();
  Serial.print("[RAW] Bit físico ");
  Serial.print(physBit);
  Serial.println(" activado en todos los dígitos.");
}
...
void printHelp() {
  ...
  Serial.println("  phys X         : registrar bit físico para segmento lógico "
                 "actual (0-7)");
  ...
  Serial.println(
      "  mode demo      : cambiar a modo demo (contador y patrones)");
  Serial.println("  num A B        : (modo demo) fija números A y B (0-999) "
                 "inmediatamente");
...
void setup() {
  ...
  Serial.println(
      "Escribe 'help' para ver comandos. Paso inicial: segmento 'a'.");
  showCurrentSegment();
  delay(1000);
  ...
  Serial.println("En modo diag auto cada segundo cambia el segmento. Orden "
                 "lógico: a b c d e f g");
  if (includeDecimalPointInTest)
    Serial.println("(Incluyendo punto decimal al final)");
  Serial.println(
      "Si la asignación no coincide, edita segmentMap y vuelve a cargar.");
  printHelp();
}
    ...
    if (c == '\n') {
      ...
      if (cmdLine.length()) {
        ...
        if (lower == "h" || lower == "help") {
          ...
          autoAdvance = false;
          currentSegmentIndex =
              (currentSegmentIndex + (includeDecimalPointInTest ? 8 : 7) - 1) %
              (includeDecimalPointInTest ? 8 : 7);
          showCurrentSegment();
        } else if (lower == "a" || lower == "auto") {
          autoAdvance = !autoAdvance;
          Serial.print("[INFO] Modo auto = ");
          Serial.println(autoAdvance ? "ON" : "OFF");
        } else if (lower == "inv" || lower == "invert") {
          segmentsActiveHigh = !segmentsActiveHigh;
          Serial.print("[INFO] activeHigh ahora = ");
          Serial.println(segmentsActiveHigh ? "true" : "false");
          showCurrentSegment();
          ...
          if (space > 0) {
            ...
            if (val >= 0 && val < 8) {
              ...
              Serial.print("[OK] Registrado segmento lógico ");
              Serial.print(segmentNames[logicalSegment]);
              Serial.print(" -> físico ");
              Serial.println(val);
              printMappingSummary();
          ...
          for (int i = 0; i < 8; i++)
            observedMap[i] = -1;
          Serial.println("[INFO] Mapeo observado reiniciado.");
        } else if (lower == "show") {
          showCurrentSegment();
          ...
          if (space > 0) {
            ...
            if (m == "diag") {
              ...
              Serial.println("[MODE] Diagnóstico");
              showCurrentSegment();
            } else if (m == "demo") {
              ...
              Serial.println("[ERR] mode diag|demo");
          } else
            Serial.println("Uso: mode <diag|demo>");
        } else if (lower.startsWith("num")) {
          if (currentMode != MODE_DEMO) {
            Serial.println("[ERR] num sólo en modo demo");
          } else {
            int firstSpace = lower.indexOf(' ');
            if (firstSpace > 0) {
              String rest = lower.substring(firstSpace + 1);
              ...
              if (space2 > 0) {
                ...
                Serial.println("Uso: num <A 0-999> <B 0-999>");
            } else
              Serial.println("Uso: num <A> <B>");
          }
        } else if (lower.startsWith("speed")) {
          if (currentMode != MODE_DEMO) {
            Serial.println("[ERR] speed sólo en modo demo");
          } else {
            int space = lower.indexOf(' ');
            if (space > 0) {
              ...
              if (ms >= 50) {
                demoInterval = ms;
                Serial.print("[SPEED] Intervalo demo = ");
                Serial.println(demoInterval);
              } else
                Serial.println("[ERR] speed mínimo 50 ms");
            } else
              Serial.println("Uso: speed <ms>");
          }
        } else if (lower == "resetdemo") {
          if (currentMode == MODE_DEMO) {
            demoCounter = 0;
            lastDemoUpdate = 0;
            Serial.println("[INFO] Demo reiniciada");
          } else
            Serial.println("[ERR] resetdemo sólo en modo demo");
        } else {
