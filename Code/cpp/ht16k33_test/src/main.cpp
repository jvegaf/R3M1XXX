...
void displayLedInfo(uint8_t ledNum) {
  ...
  Serial.print(F("LED #"));
  if (ledNum < 100) Serial.print(F(" "));
  if (ledNum < 10) Serial.print(F(" "));
  Serial.print(ledNum);
  Serial.print(F(" | Row: "));
  if (row < 10) Serial.print(F(" "));
  Serial.print(row);
  Serial.print(F(", Col: "));
  Serial.print(col);
  Serial.print(F(" | RAM["));
  if (ramByte < 10) Serial.print(F(" "));
...
void displayKeyInfo(int8_t keyValue) {
  if (keyValue > 0) {
    ...
    if (keyNum < 10) Serial.print(F(" "));
    Serial.print(keyNum);
    Serial.print(F(" | Group: "));
    Serial.print(group);
    Serial.print(F(", Position: "));
    if (keyInGroup < 10) Serial.print(F(" "));
...
void displayKeyRaw() {
  ...
  for (uint8_t group = 0; group < KEY_GROUPS; group++) {
    ...
    Serial.print(group);
    Serial.print(F(": 0x"));
    if (keyData[group] < 0x1000) Serial.print(F("0"));
    if (keyData[group] < 0x100) Serial.print(F("0"));
    if (keyData[group] < 0x10) Serial.print(F("0"));
    ...
    for (int8_t bit = 12; bit >= 0; bit--) {
      ...
      if (bit % 4 == 0 && bit != 0) Serial.print(F(" "));
    }
    Serial.println();
  }
  Serial.println();
